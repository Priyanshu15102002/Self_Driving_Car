class Car {
    constructor(x, y, width, height, controlType, maxSpeed = 3) {
        ...
        if (controlType !== "DUMMY") {
            ...
            this.brain = new NeuralNetwork([this.sensor.rayCount, 6, 4]); // '6' is a hidden neural network layer, '4' is for the up, down, left or right binary output
        }
        this.controls = new Controls(controlType);
    }

    update(roadBorders, traffic) {
        // Car can move if its not damaged
        if (!this.damaged) {
            this.#move();
